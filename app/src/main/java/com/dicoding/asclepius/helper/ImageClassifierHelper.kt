...
class ImageClassifierHelper(
   ...
   fun classifyStaticImage(imageUri: Uri) {
      ...
      val bitmap = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {val source = ImageDecoder.createSource(context.contentResolver, imageUri)
         ImageDecoder.decodeBitmap(source)
      } else {
         MediaStore.Images.Media.getBitmap(context.contentResolver, imageUri)
         throw UnsupportedOperationException("Image loading not supported on devices below API level 29")
